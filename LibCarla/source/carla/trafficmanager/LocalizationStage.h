...
namespace traffic_manager {
  ...
  /// Lane change details: based on normalized vehicle presence
  std::map<SegmentId, uint32_t> laneVehicleMap;

  /// A simple method used to draw waypoint buffer ahead of a vehicle.
  void DrawBuffer(Buffer &buffer);

  /// Get next selected waypoint based on minimal traveled path
  SimpleWaypointPtr getPossibleNextWayPoint(std::vector<SimpleWaypointPtr> &next_waypoints);
